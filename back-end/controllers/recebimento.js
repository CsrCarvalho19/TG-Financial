...
controller.novo = async (req, res) => {
    ...
    try {
        await Recebimento.create(req.body)
        // HTTP 201: Created
        res.status(201).end()
    }
    catch(erro) {
...
controller.listar = async (req, res) => {
    try {
        let dados = await Recebimento.find() // Traz todos os recebimentos cadastrados
...
controller.obterUm = async (req, res) => {
    try {
        ...
        let obj = await Recebimento.findById(id)
...
controller.atualizar = async (req, res) => {
    try {
        ...
        let ret = await Recebimento.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
    try {
        ...
        let ret = await Recebimento.findByIdAndDelete(id)
